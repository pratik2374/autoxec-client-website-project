...
export const quickRead = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: 'Author Name',
      type: 'string',
      description: 'Optional override for author display name.',
    }),
    defineField({
      name: 'authorBio',
      title: 'Author Bio',
      type: 'text',
      rows: 3,
      description: 'Optional bio to appear at the bottom of the article.',
    }),
    defineField({
      name: 'relatedPostsStrategy',
      title: 'Related Posts Strategy',
      type: 'string',
      options: { list: [
        {title: 'Auto (System selects matching)', value: 'auto'},
        {title: 'Manual Selection', value: 'manual'},
        {title: 'None (Hide related posts block)', value: 'none'}
      ], layout: 'radio' },
      initialValue: 'auto',
    }),
    defineField({
      ...
      type: 'array',
      of: [{ type: 'reference', to: [{ type: 'article' }, { type: 'quickRead' }] }],
      validation: (Rule) => Rule.max(3),
      hidden: ({ parent }) => parent?.relatedPostsStrategy !== 'manual',
    }),
  ],
  preview: {
    select: {
